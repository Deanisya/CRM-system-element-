...
function CallItem({ call: { id, date, time, responsible, type, priority } }: CallItemProps) {
	const { handleUpdateName, handleUpdateType, handleUpdatePriority, deleteEvent, handleUpdateDateAndTime } = useCallUpdater(id);
	console.log('date:', date);
	console.log('time:', time);
	console.log('new Date:', new Date(`${date}T${time}`));
	...
	return (
		<div className='bg-dark-blue-bg rounded-full pb-4 pt-4 px-12'>
			...
			<GridContainer>
				...
				{/* <div className='bg-[#6A7B96] rounded-full py-3 px-6 w-[164px] h-[96px] first:justify-self-start'> */}{' '}
				<DatePicker selected={new Date(`${date}`)} timeInputLabel='Time:' dateFormat='MM/dd/yyyy h:mm' showTimeInput />
				{/* </div> */}
