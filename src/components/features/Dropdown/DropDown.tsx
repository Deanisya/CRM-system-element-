...
function DropDown<T extends string>({ options, initial, text }: DropDownProps<T>) {
	const { ref, isOpen, select, toggle, handleSelect } = useDropdown<T>(initial);

	return (
		<div className='relative w-fit mb-1.5' ref={ref}>
			<DropDownButton select={select} onClick={toggle}>
			...
			{isOpen && <DropDownOptionsList options={options} text={text} onSelect={handleSelect} />}
		</div>
	);
}
